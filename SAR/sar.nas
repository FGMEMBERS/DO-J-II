...
var this_alt_correction =  2.6; # an this is the DO-J-II correction
...
## DO-J-II can't heave other aircrafts, so we doesn't need this part ##
#
#var my_poid = func (){
...
var rescue_me = func(v) {
    ...
    if (v != nil and v >= 0){
      ...
      if(yourLocked){
         ...
         var gr     = getprop("/ai/models/multiplayer["~v~"]"~agl_float) or 0;
         var ground = gr - sar_alt_correction + this_alt_correction - towel_length; 
...
###############################################################################
################# VERY BAD HACK help H-21C to see the DO-J ####################
###############################################################################
# dieser Part muss wieder entfernt werden, wenn das Problem 
# (evtl. mit den Menues) gefunden wurde.
# Ich gehe im Augenblick davon aus, dass das doppelte vorhanden sein von
# dynamischen Menues (MP-Copilot/Pilot und SAR) die Stoerung verursacht.
# Immer wenn eine Do-J sich anmeldet, kann die H21-C sie kurz sehen.
# Sobald die DO-J aber fdm-initialized ist, wird sie für die H21-C unsichtbar.
# Erst ein Auswaehlen der H21-C als SAR Rettung, aus der DO-J heraus gewaehlt,
# laesst die DO-J dann selbst der H21-C wieder sichtbar werden.
# Also lasse ich hier die DO-J alle SAR-H21-C kurz bestaetigen 
# und wieder abmelden.
###############################################################################
...
#            var type = pilot.getNode("sim/model/path").getValue();
#
#            if (type == "Aircraft/H-21C-Shawnee/Models/H21-piasecki.xml") {
#                settimer ( func { setprop("/sar_service/sar-callsign", pilot.getNode("callsign").getValue());}, 2);
#                settimer ( func { setprop("/sar_service/sar-callsign", "");}, 3);
#            }
#
#        }
#    }
